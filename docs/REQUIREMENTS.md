...
| NFR2 | Security | CSP headers configured in `firebase.json`. | 🟡 | Header exists (`hosting.headers`, 2026-07-12): `script-src`/`connect-src`/`img-src` allowlisted to the app's actual external hosts, `frame-ancestors`/`object-src`/`base-uri`/`form-action` fully locked down. **Not fully closed:** `script-src`/`style-src` still require `'unsafe-inline'` because every page has inline `<script type="module">` blocks and Tailwind's CDN injects runtime CSS — see `ARCHITECTURE.md`'s CSP section for the full tradeoff and what a real fix requires |
